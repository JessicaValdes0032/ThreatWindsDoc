...
\
When building an entire application around an API, the usage of authorization headers becomes a common approach to control user sessions and ensure the security of the application. Once the client is authenticated, the authorization header is included in the HTTP request headers to control the user's session. This header contains a unique token that is generated by the server and sent to the client as a means to identify the user's session. The authorization header remains valid as long as the user's session is active and has not expired, providing a secure and reliable method to maintain user sessions within the application.\
\
...
> { "sessionID": "72dceecb-8d12-40b2-8b33-b60754134364", **"bearer": "m5orsH7hqidILzwkGUcjf9gl93r9KqOj"**, ... }

Example of KeyPair when you create a new one:

> { **"apiKey": "fq6JoEFTsxiXAl1cVxPDnK4emIQCwaUB"**, **"apiSecret": "fq6JoEFTsxiXAl1cVxPDnK4emIQCwaUBfq6JoEFTsxiXAl1cVxPDnK4emIQCwaUB"**, "expireAt": 1674492894, ... }

See [QAuth](AUTENTICATIONAPI.md) for more info.\


> _**IMPORTANT:**_ If you choose to use the Authorization header for authentication, you do not need to include your API key and secret as query parameters in the request. Conversely, if you include your API key and secret as query parameters, you do not need to use the Authorization header.
...
**STEP 1** The first thing you need to know is which endpoint to use. In this case, since you want to retrieve the latest records, you should use the [HISTORY](search/history.md) endpoint. To do this, you will need to make a POST request to https://intelligence.threatwinds.com/api/search/v1/entities/history.
...
* The **most-active-malwares** aggregation uses the [terms](search/query-and-aggregations/aggregations.md#terms) aggregation to group the malware reports by their name, and then returns the top 50 most frequent ones.
* The **malware\_per\_minut**e aggregation uses the [date\_histogram](search/query-and-aggregations/aggregations.md#histogram) aggregation to group the malware reports by day. The **query** section use a [filter](search/query-and-aggregations/query.md#filter) the results to only include malware reports within the last 5 minutes.
